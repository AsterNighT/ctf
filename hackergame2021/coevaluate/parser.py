...
def lexical_analysis(s):
    ...
    for c in s:
        if c in mappings:
            ...
            token = Node(TokenType.T_NUM, value=c)
        else:
...
def parse_e2(tokens):
    ...
    while tokens[0].token_type in [TokenType.T_MULT, TokenType.T_MOD, TokenType.T_DIV]:
        node = tokens.pop(0)
        node.children.append(left_node)
...
def parse_e3(tokens):
    if tokens[0].token_type == TokenType.T_MINUS:
        node = tokens.pop(0)
        node.children.append(Node(TokenType.T_NUM, value='0'))
        node.children.append(parse_e3(tokens))
        return node
