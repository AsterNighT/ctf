...
key = open(os.path.join(file_dir, 'key.txt'), 'r').read()
assert(28 <= len(key) <= 36)
...
banner = """
*********************** AP ***********************
I am a cute AP, input your packet and I will test
to decide if it is valid. 
**************************************************
Your packet:
"""
print banner
...
data = raw_input()          # 请输入十六进制串，否则就会报错
try:
    data = data.decode('hex')   # 得到ASCII串
except TypeError:
    print "invalid input"
    sys.stdout.flush()
    exit(1)

data = xor(data, key.decode('hex')) # 解密

length = len(data)          
if(length < 5):             # 不满足 crc 最小长度
    print "invalid input"
    exit(1)

crc_str = data[length-4:]
crc_str = char2hexs(crc_str[3]) + \
          char2hexs(crc_str[2]) + \
          char2hexs(crc_str[1]) + \
          char2hexs(crc_str[0])

text = CRC32(data[:length-4]) % (1<<32)

if(text == int(crc_str, 16)):
    print "valid input"
else:
    print "invalid input"
sys.stdout.flush()
